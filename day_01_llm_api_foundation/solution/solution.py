...
def compare_models(prompt: str) -> dict:
    ...
    # Cost estimate: (words / 0.75) / 1000 * cost_per_1k_tokens
    gpt4o_tokens_estimate = len(gpt4o_response.split()) / 0.75
    gpt4o_cost_estimate = (gpt4o_tokens_estimate / 1000) * COST_PER_1K_OUTPUT_TOKENS["gpt-4o"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        gpt4o_resp = truncate(r.get("gpt4o_response", "").replace("\n", " "), 40)
        mini_resp = truncate(r.get("mini_response", "").replace("\n", " "), 40)
        gpt4o_lat = f"{r.get('gpt4o_latency', 0):.3f}s"
        mini_lat = f"{r.get('mini_latency', 0):.3f}s"

        line = f"{prompt:<42} | {gpt4o_resp:<42} | {mini_resp:<42} | {gpt4o_lat:<14} | {mini_lat:<12}"
        lines.append(line)
